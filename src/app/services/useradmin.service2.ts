...
export class UseradminService {
  ...
  public getControlVariables(): void {
    this.https.get('../assets/control-variable.json').subscribe(result => {
      this.controlVariables = result;
    });
  }
  ...
  getUser() {
    return this.https.get('https://' + this.domain_name + '/rest/v1/secured?V_SRC_CD=' + this.V_SRC_CD + '&V_CD_TYP=USER&REST_Service=Masters&Verb=GET');
  }
  ...
  */
  public downloadFile(fileName: any) {
    ...
    this.http.post('https://' + this.removeSubDomain(this.domain_name) + '/FileAPIs/api/file/v1/download',
      formData, option).subscribe(data => {
