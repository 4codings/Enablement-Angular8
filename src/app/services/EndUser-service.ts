...
export class EndUserService {
    ...
    */
    getAllExecutablesForSelctedExecutableType(){
        return this.http.get(this.baseSecureJsonUrl + "V_SRC_CD=" + this.V_SRC_CD + "&V_EXE_TYP=E_REST" +"&V_USR_NM" +this.V_USR_NM + "&REST_Service=UsersExe&Verb=GET", this.apiService.setHeaders());
    }
    
    /*
        To fetch Input/Output for selected executable 
    */
    getInputOutputForSelctedExecutable(V_UNIQUE_ID : string){
        // TODO V_UNIQUE_ID should be populated dynamically
        return this.http.get(this.baseSecureJsonUrl + "V_UNIQUE_ID=" + "28190" + "&V_EXE_TYP=E_REST" + "&V_EXE_CD=LoadFPDSCSVFile" + "&V_SRC_CD=" + this.V_SRC_CD +  "&V_USR_NM=" + this.V_USR_NM  +"&REST_Service=Exe&Verb=GET", this.apiService.setHeaders());
    }        
}
